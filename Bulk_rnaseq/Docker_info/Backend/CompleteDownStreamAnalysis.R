...
ggsave(filename = file.path(output_dir, paste0(file_base_name, "_pca_plot.png")), plot = p, width = 8, height = 6)
...
p_filtered <- ggplot(pca_data_filtered, aes(x = PC1, y = PC2, color = Group)) +
  ...
  theme_minimal() +
  theme(legend.title = element_blank())
ggsave(filename = file.path(output_dir, paste0(file_base_name, "_pca_plot_filtered.png")), plot = p_filtered, width = 8, height = 6)

# Rimuove le righe con valori di conteggio totali pari a 0 nella matrice filtrata
