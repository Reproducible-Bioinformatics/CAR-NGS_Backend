...
# Funzione per eseguire l'analisi DESeq2 con Docker
esegui_deseq2_docker() {
    ...
    docker run --rm \
        -v "$DATA_DIR:/scratch" \
        "$DOCKER_IMAGE" \
        Rscript /home/Deseq2.R gene_count_matrix.csv Covariatesstat.csv wt Drosophilamelanogaster
}
