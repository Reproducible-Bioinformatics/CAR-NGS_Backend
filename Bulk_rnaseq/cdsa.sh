...
# Funzione per eseguire l'analisi completa downstream con Docker
esegui_complete_downstream_analysis_docker() {
    ...
    docker run --rm \
        -v "$DATA_DIR:/scratch" \
        "$DOCKER_IMAGE" \
        Rscript /home/CompleteDownStreamAnalysis.R gene_count_matrix.csv Covariatesstat.csv wt Drosophilamelanogaster
}
